...
# delete conf
curl -X DELETE "http://localhost:${PORT}/solr/admin/cluster/configs/static_schema_1_6"

# create conf
(cd solr/solr/${CONFIGSETS}/sunspot/conf/ && zip -r - *) | curl -X POST --header "Content-Type:application/octet-stream" --data-binary @- "http://localhost:${PORT}/solr/admin/configs?action=UPLOAD&name=static_schema_1_6"
