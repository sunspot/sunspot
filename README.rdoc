...
{<img src="http://travis-ci.org/sunspot/sunspot.png" />}[http://travis-ci.org/sunspot/sunspot]
...
=== Working with spatial searches:

  class Store
    searchable do
      latlon :location do
        Sunspot::Util::Coordinates.new(lat, long)
      end
    end
  end
  ...
  Store.search do |search|
    search.spatial(:location, lat, lon, :radius => 5)
  end
