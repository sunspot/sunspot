...
case $GEM in
  "sunspot")
    ...
    cd sunspot
    /bin/echo -n "Starting Solr on port 8982 for Sunspot specs..."
    bundle install --quiet --path vendor/bundle
    if [ -f sunspot-solr.pid ]; then bundle exec sunspot-solr stop || true; fi

    export SOLR_URL=http://localhost:8982/solr
    bundle exec sunspot-solr start -p 8982 -d /tmp/solr
    wait_until_solr_responds 8982
    /bin/echo "done."
